...
public class GlobalGrid : MonoBehaviour {
	...
	public static bool TryMove(List<IGridOccupant> occupants, Vector2Int direction, bool pushAllowed = true) {
		...
		//unregister all
		foreach (var gridOccupant in occupants) {
			foreach (var occupantPosition in gridOccupant.Positions) {
				GridOccupants.Remove(occupantPosition);
			}
		}
		...
		foreach (var gridOccupant in occupants) {
			foreach (var occupantPosition in gridOccupant.Positions) {
				GridOccupants[occupantPosition] = gridOccupant;
			}
		}
		return true;
	}
	...
	private static bool TryMoveRecursive(IGridOccupant occupant, List<IGridOccupant> trackedOccupants, Vector2Int direction, bool allowPush = true) {
		foreach (var oPos in occupant.Positions) {
			Vector2Int desiredPos = oPos + direction;
			//check if position is free
			...
			trackedOccupants.Add(nextOccupant);
			if (!allowPush || !TryMoveRecursive(nextOccupant, trackedOccupants, direction)) {
				//it cant move, abort
				return false;
			}
		}
